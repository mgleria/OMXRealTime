...
/* Objeto para acceder al driver del LCD */
...
static void prvLCDPutString( char *pcString )
{
    uint16_t size = sizeof(pcString);
    LCD_PutString(pcString,size);
...
xLCDMessage xMessage;
	...
	/* Welcome message. */
	prvLCDPutString( "OMIXOM" );

	for( ;; )
	{
		/* Wait for a message to arrive that requires displaying. */
		while( xQueueReceive( xLCDQueue, &xMessage, 0 ) != pdPASS );
