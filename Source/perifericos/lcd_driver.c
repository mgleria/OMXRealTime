...
/*********************************************************************
 * Function: void LCD_ChangeRow(uint8_t);
 *
 * Overview: Cambia el cursor a la primer posición de la fila indicada.
 *
 * PreCondition: ??
 *
 * Input: Fila deseada (0 ó 1)
 *
 * Output: None
 *
 ********************************************************************/
void LCD_ChangeRow ( uint8_t toRow )
{
    if (toRow == 0)
        {   
            LCD_SendCommand ( LCD_COMMAND_RETURN_HOME, LCD_F_INSTR ) ;
            row = 0 ;
        }
        else
        {
